...
use neothesia_core::{
    ...
    piano_layout,
    render::{
        BgPipeline, GlowRenderer, GuidelineRenderer, KeyboardRenderer, NoteLabels, QuadRenderer,
        QuadRendererFactory, TextRenderer, TextRendererFactory, WaterfallRenderer,
    },
};
...
impl Recorder {
    fn new(args: &cli::Args) -> Self {
        ...
        // Create background animation pipeline
        let bg_pipeline = BgPipeline::new(&gpu);
        ...
        let glow = GlowRenderer::new(&gpu, &transform_uniform, &keyboard_layout);
    ...
    fn update(&mut self, delta: Duration) {
        ...
        // Update glow for pressed keys
        self.glow.clear();
        let keyboard_layout = self.keyboard.layout();
        let keyboard_pos = self.keyboard.pos();
        for (id, key_state) in self.keyboard.key_states().iter().enumerate() {
            if let Some(color) = key_state.pressed_by_file().or(key_state.pressed_by_user()) {
                let key = &keyboard_layout.keys[id];
                self.glow.push(
                    id,
                    *color,
                    keyboard_pos.x + key.x(),
                    keyboard_pos.y,
                    key.width(),
                    delta,
                );
    ...
    fn render(
        ...
        // Clear to black - background animation will provide the actual background
...

        {
            ...
            self.bg_pipeline.render(&mut rpass);
            ...
            // Render glow on top of keyboard
            self.glow.render(&mut rpass);
