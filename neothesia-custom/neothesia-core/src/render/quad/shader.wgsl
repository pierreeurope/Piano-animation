...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var final_color = in.quad_color.xyz;
    
    // Detect if this is a colored (pressed) key vs white/black unpressed key
    let is_colored = (in.quad_color.x > 0.1 && in.quad_color.y < 0.95) || 
                     (in.quad_color.y > 0.1 && in.quad_color.x < 0.95) ||
                     (in.quad_color.z > 0.1 && in.quad_color.x < 0.95);
    
    if is_colored {
        // Calculate position relative to quad
        let rel_pos = (in.position.xy - in.quad_position) / in.quad_size;
        
        // Add inner glow - brightest at top, fading down
        let top_glow = (1.0 - rel_pos.y) * 0.4;
        final_color = final_color + vec3<f32>(top_glow);
        
        // Add highlight at top edge
        let edge_highlight = smoothstep(0.15, 0.0, rel_pos.y) * 0.5;
        final_color = final_color + vec3<f32>(edge_highlight);
        
        // Add slight side highlights
        let left_highlight = smoothstep(0.1, 0.0, rel_pos.x) * 0.15;
        let right_highlight = smoothstep(0.9, 1.0, rel_pos.x) * 0.1;
        final_color = final_color + vec3<f32>(left_highlight + right_highlight);
        
        // Boost saturation and vibrancy
        let luminance = dot(final_color, vec3<f32>(0.299, 0.587, 0.114));
        final_color = mix(vec3<f32>(luminance), final_color, 1.3);
        
        // Clamp to valid range
        final_color = clamp(final_color, vec3<f32>(0.0), vec3<f32>(1.0));
    }

    return vec4(final_color, in.quad_color.w * alpha);
}
